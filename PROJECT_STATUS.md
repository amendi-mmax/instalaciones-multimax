...
**Limitación crítica, reportada con máxima visibilidad, no resuelta en esta ronda**: `admins`/`coordinadores`/`empresas`/`tiendas` tienen RLS habilitado sin policies de `SELECT` para `authenticated` (auditado desde Sprint 4.0.1/4.1.1) -- bloquea la resolución de perfil real para los roles `admin`/`coordinador`. Con la configuración de base de datos actual, **solo el login de `instalador` puede probarse de punta a punta contra Producción real**; `admin`/`coordinador` autentican correctamente pero son expulsados con "perfil no encontrado" hasta que se agreguen esas policies (fuera del alcance pedido en este Sprint -- ninguna migración/RLS fue solicitada). Ver sección 8 del informe completo para la recomendación técnica.

Mismo bloqueo de red/ausencia de `node_modules/` de siempre -- `npm run lint`/`typecheck`/`build`/`dev` no se pudieron ejecutar en este entorno. Se usó, por primera vez en este proyecto, una instalación **global** de TypeScript (`tsc` 6.0.3, no forma parte de las dependencias reales del proyecto) para correr `tsc --noEmit` sobre los 19 archivos nuevos/modificados -- los únicos diagnósticos encontrados son artefactos conocidos de la falta de `node_modules` (módulos no encontrados, JSX sin `@types/react`), verificados cruzando contra el mismo patrón de diagnósticos en archivos ya aprobados del repositorio.

## Qué falta

- **Bloqueante para cerrar el Sprint 4.2.1 (crítico)**: `admins`/`coordinadores`/`empresas`/`tiendas` no tienen policies RLS de `SELECT` para `authenticated` -- decisión del usuario requerida sobre si agregar esas policies (migración nueva) antes de poder validar login de `admin`/`coordinador` de punta a punta. Ver `docs/architecture/frontend/SPRINT_4_2_1_AUTH_REPORT.md §8`.
- **Bloqueante para cerrar el Sprint 4.2.1**: ejecutar en el entorno del usuario `npm run lint && npm run typecheck && npm run build && npm run dev` en verde, y probar el login real contra Producción (al menos con un `instalador`, el único rol no bloqueado por el punto anterior).
