...
**Desde el Sprint 3.2**, cada Sprint se identifica por el bloque/selector real del HTML (no por un nombre genérico de sección como "Sidebar"/"Navigation"/"Dashboard"). La columna "Elemento" de los Sprints 3.6 en adelante todavía usa los nombres genéricos originales porque no se ha hecho el análisis previo obligatorio de esos bloques — se actualizarán al selector real del HTML cuando le corresponda su turno, sin adelantar ese análisis ahora. El Sprint 3.4 confirmó, por inspección directa del HTML (no por el nombre genérico que tenía esta tabla, "Main Layout"), que el bloque real pendiente era `mx-suc-sel`; el Sprint 3.5 verificó que "Publish Modal" sí correspondía al bloque real (`PublishModal`) — ver notas abajo.
...
| 3.5 | 🟡 En revisión — pendiente validación local + visual del usuario | `PublishModal` (formulario de publicar trabajo) | `feature/sprint-3-5-publish-modal` |
...
- **Sprint en revisión**: 3.5 (`PublishModal`, formulario de publicar trabajo). Análisis, componente (`PublishModal`), constantes (`PROVINCIAS`/`ZONAS`/`BID_OPTIONS`/`SLOTS_COORD`), CSS e integración visual completos; validación best-effort en verde. Pendiente de validación real (`npm install/lint/typecheck/build/dev`) y validación visual del usuario antes de poder marcarse ✅ Completado.
- **Próximo Sprint a desarrollar**: 3.6 (Job Cards, nombre genérico pendiente de análisis real del bloque HTML). **No se inicia sin aprobación explícita del usuario.**
- **Integraciones temporales activas en `src/layouts/RootLayout.tsx`** (a retirar cuando existan los layouts reales por rol): `SucursalSelect` + `MxSubtabs` (en ese orden) para `role === 'coordinador'` (Sprint 3.4 + 3.3), `mx-instwrap`/`InstallerSidebar` (con Phone Placeholder reservado) para `role === 'instalador'` (Sprint 3.2.1/3.2.2), y `PublishModal` (Sprint 3.5) como hermano de ambos bloques de `role` — igual que en el HTML fuente, `PublishModal` no depende de ningún `role` específico, solo de `showPublishModal` (forzado a `true` temporalmente).
- **Componentes creados sin consumidor todavía**: ninguno pendiente de `mx-subtabs`/`mx-instside`/`mx-suc-sel`/`PublishModal` (los cuatro ya integrados); sigue pendiente de decisión la posible duplicación entre `ui/Tabs` (Fase 3, Radix, sin consumidores) y `MxSubtabs`/`MxSubtabButton` para cuando se construya AdminPanel.
- **Problemas conocidos, reportados sin corregir**: (1) el badge de sucursal de `HeaderStatus` (`sucursalActiva`, default `"Multiplaza"` desde Sprint 3.1) no recibe el `sucursalCoord` real que ahora controla `SucursalSelect` (Sprint 3.4) — quedan desincronizados; no se corrige porque requeriría modificar la invocación de `Header`, fuera del alcance mínimo permitido en el Sprint 3.4. (2) El botón "Publicar trabajo" de `PublishModal` (Sprint 3.5) invoca `onPublish` con una función vacía — no existe todavía ningún `TRABAJOS`/lista de trabajos que actualizar. Ver `docs/sprints/sprint-3.4.md` y `docs/sprints/sprint-3.5.md` respectivamente.
...
- El bloque `mx-instside` migrado en 3.2 es hermano de `.mx-phone` dentro de `.mx-instwrap` — ese wrapper y el teléfono (`mx-phone`/`mx-phone-bar`/`mx-phonetabs`) siguen sin componente propio (`layouts/InstallerLayout.tsx`, según `ARCHITECTURE.md` §3, no existe todavía). Ningún Sprint de los 3.6-3.16 listados originalmente cubre explícitamente ese layout — a confirmar con el usuario cuándo/cómo se planifica.
