...
**Migraciones aplicadas en Producción vía MCP** (previas a esta ronda de implementación, sin volver a aplicarse): `0015_coordinadores_service_role_grant.sql` (`GRANT SELECT, INSERT, UPDATE, DELETE ON public.coordinadores TO service_role` -- mismo gap ya resuelto una vez para `admins`/`instaladores` en `0003`) y `0016_admins_select_coordinadores.sql` (policy RLS `"admins ven coordinadores de su empresa"`, reutilizando `es_admin_de_empresa()` ya creada en `0014` -- sin duplicar lógica). Ambas verificadas en vivo vía MCP, sin discrepancias, sin acceso entre empresas.

**Edge Function** (`supabase/functions/admin-operations/index.ts`, MODIFICADA, desplegada): 3 acciones nuevas en el mismo `switch` ya existente -- `list_coordinadores`, `invite_coordinador`, `set_coordinador_activo`. Sin gate de `es_principal` (a diferencia de `invite_admin`/`set_admin_activo`): la matriz de permisos C2 ya documentada establece que gestionar coordinadores es igual para Administrador Principal y Secundario. `invite_coordinador` valida server-side que `tienda_id` pertenece a `admin.empresa_id` (nunca confía en el `<select>` del frontend) antes de invitar; `rol` se fuerza siempre a `'coordinador'`, `activo` nace `true` (mismo criterio que `invite_admin`, sin paso de verificación de documentos como instaladores). `list_coordinadores` es una decisión técnica nueva de este Sprint: como `public.coordinadores` no tiene columna `email` (confirmado contra el schema real), el listado se sirve desde la Edge Function en vez de un `SELECT` directo del cliente -- resuelve el correo de cada coordinador vía Auth Admin API (`serviceRoleClient.auth.admin.getUserById()`), la única forma segura de hacerlo sin agregar una columna nueva ni exponer `auth.users` a un cliente `authenticated`.

**Frontend**: `src/types/admin-operations.ts` (MODIFICADO, +`InviteCoordinadorPayload`/`SetCoordinadorActivoPayload`/`CoordinadorConEmail`); `src/services/admin-operations.service.ts` (MODIFICADO, +`listCoordinadores`/`inviteCoordinador`/`setCoordinadorActivo`, mismo `invokeAdminOperation()` genérico ya usado por instaladores/admins); `src/components/shared/admin-coordinadores.tsx` (NUEVO) -- mismo patrón estructural/visual que `admin-administradores.tsx` (tabla + formulario de invitación + toggle de estado + Toast local, clases `.mx-admintable`/`.mx-adminrow*`/`.mx-admin-act`/`.mx-invite*`, sin CSS nuevo), con un `<Select>` de tienda adicional (mismo patrón que "Empresa/taller" en `admin-instaladores.tsx`, poblado con `tiendasRepository.getByEmpresaId()`, ya existente sin cambios). `admin-panel.tsx` (MODIFICADO) -- sexta pestaña "Coordinadores" (ícono `UserCog`), mismo patrón `MxSubtabs`/`MxSubtabButton`.

**Repositorios**: sin cambios -- `coordinadoresRepository`/`tiendasRepository` ya tenían todo lo necesario (`getByEmpresaId`/`getByTiendaId`), confirmado sin necesidad de métodos nuevos. `coordinadoresRepository` no se usa para el listado del Admin (por el motivo de `email` explicado arriba) pero permanece intacto para cualquier otro consumo.

**Sin edición**: el módulo de Administradores (la referencia explícita de este Sprint) tampoco tiene ningún mecanismo de edición -- no existe un patrón seguro que replicar sin inventar arquitectura nueva. MVP priorizado: listar, invitar, activar/desactivar. **Sin reenvío de invitación**: Administradores tampoco lo tiene -- no implementado, mismo criterio.

`npm run typecheck`/`npm run build`/`npm run lint` limpios (mismos 3 warnings preexistentes -- `AuthContext.tsx`/`useRealtime.ts`/`supabase/client.ts` -- ninguno en archivos de este Sprint). **Sin invitación real enviada** (sin correo de prueba autorizado explícito en este entorno) -- pendiente de validación funcional real por el usuario. Sin `git commit`/`push`.

## [Sprint 9.2 — Consolidación y verificación de operaciones administrativas] — 2026-08-20 — 🟢 Verificado, sin brechas — DETENIDO PARA REVISIÓN

**Este Sprint NO implementó funcionalidad nueva.** Su objetivo era auditar/verificar la capa de operaciones administrativas ya construida (Sprints A/B/C, commits previos en esta misma rama) contra los requisitos explícitos de Sprint 9.2 -- conclusión de la Fase 1 (análisis obligatorio, aprobada por el usuario antes de tocar nada): las 7 operaciones/protecciones mínimas pedidas (invitación/activación-desactivación de Secundario, consulta, protección del Principal, protección del último Principal, aislamiento por empresa, autorización por rol) ya estaban implementadas y desplegadas (`admin-operations` v7). Cero archivos de aplicación modificados (`admin-administradores.tsx`/`admin-panel.tsx`/`admin-operations.service.ts`/`admin-operations/index.ts` intactos, confirmado con `git status`/`git diff` antes y después).

**9 pruebas técnicas re-ejecutadas con evidencia fresca en esta rama** (dentro de `BEGIN...ROLLBACK` cuando implicaban escritura, `0` datos de prueba persistidos, `3` admins reales sin cambios): Principal ejecuta operaciones (gate `activo && es_principal` → `true`), Secundario no puede (gate → `false`, verificado contra los 2 administradores secundarios reales ya existentes en Producción de pruebas anteriores), usuario no-admin no puede (`0` filas en `admins`), aislamiento entre empresas (empresa+admin de prueba temporal, comparación de `empresa_id` → rechazo confirmado), protección del último Principal y del Principal mismo (`UPDATE`/`DELETE` sobre el Principal real → `23514`, trigger `proteger_ultimo_admin_principal`), operación exitosa produce el cambio esperado (`toggle` real de un Secundario existente, revertido), manejo de errores (revisión de código de `admin-operations.service.ts`: `FunctionsHttpError`/`businessError`/traducción de mensajes, sin cambios), sin credenciales privilegiadas en el bundle (`dist/assets/*.js` sin ninguna coincidencia de `service_role`, único JWT presente decodifica a la clave `anon` pública).

**Auditoría RLS** (`pg_policies` sobre `admins`, fresco): 2 policies, ambas `SELECT` -- `"admins pueden leer su perfil"` (`auth.uid() = id`) y `"admins ven administradores de su empresa"` (`es_admin_de_empresa(empresa_id)`, `0014`). Sin cambios desde su aplicación. **Auditoría de GRANTs**: `authenticated` solo tiene `SELECT` sobre `admins` (sin `INSERT`/`UPDATE`/`DELETE`) -- confirmado, sin cambios. **Auditoría del trigger** (`pg_trigger`/`pg_get_functiondef`): `trg_proteger_ultimo_admin_principal`, `BEFORE UPDATE OR DELETE`, `FOR EACH ROW`, sobre `admins`, definición idéntica a `0012_proteger_ultimo_admin_principal.sql`.

`npm run lint`/`typecheck`/`build` limpios (mismos 3 warnings preexistentes, ninguno en archivos de este Sprint). `package.json` sin script `test` -- documentado, no inventado. Sin migraciones nuevas, sin cambios de Docker/Nginx/Traefik/infraestructura. Sin `git commit`/`push`.

## [Gestión de Administradores y Coordinadores — Módulo CRUD de Administradores] — 2026-08-19 — 🟢 Implementado, compilando — DETENIDO PARA REVISIÓN

Primera UI real del módulo "Gestión de usuarios" -- quinta pestaña "Administradores" en `AdminPanel` (ícono `ShieldCheck`), usando **exclusivamente** los contratos ya validados en el Sprint C: `adminsRepository.getByEmpresaId()` (listado, RLS `0013`/`0014`) + `adminOperationsService.inviteAdmin()`/`.setAdminActivo()` (Edge Function `admin-operations` v7, ya validada extremo a extremo). Sin tocar RLS, migraciones ni Edge Functions.

**`src/components/shared/admin-administradores.tsx`** (NUEVO) -- mismo patrón estructural/visual que `admin-instaladores.tsx` (tabla + formulario de invitación + toggle de estado + Toast local, clases `.mx-admintable`/`.mx-adminrow*`/`.mx-admin-act`/`.mx-invite*` ya existentes, sin CSS nuevo). Listado muestra badge "Principal"/"Secundario" + "Activo"/"Inactivo" por fila. "Invitar administrador"/"Activar-desactivar" solo se muestran si `profile.esPrincipal === true` (matriz de permisos aprobada, C2) -- **gate únicamente de UX**: la autorización real vive en `admin-operations`, que rechaza con `403` a un Secundario o cualquier intento sobre el Principal con o sin este botón visible. La fila del propio Principal nunca muestra el botón de activar/desactivar.

**Gap encontrado y cerrado**: `Perfil` (`types/perfil.ts`) no exponía `es_principal` -- se agregó `esPrincipal: boolean | null` (`null` para `coordinador`/`instalador`, mismo criterio que `documentosOk`) y `profile.service.ts` (rama `admins`) lo resuelve desde la fila ya consultada (`select('*')`, sin query nueva). Necesario para que la UI sepa si el admin actual es Principal sin una consulta adicional.

Sin pestaña de "Coordinadores" todavía -- no existe backend real para esa entidad (Sprint D, pendiente). `npm run lint`/`typecheck`/`build` limpios (mismos 3 warnings preexistentes, ninguno en archivos de este Sprint). **No se probó visualmente en navegador** (este entorno de trabajo no tiene acceso a un navegador) -- pendiente de validación visual por el usuario antes de considerar el módulo completamente cerrado. Sin `git commit`/`push`.

## [Gestión de Administradores y Coordinadores — Sprint C "Contrato frontend/backend"] — 2026-08-19 — 🟢 Validado extremo a extremo — DETENIDO PARA REVISIÓN

Cierra el ciclo Sprint A → B → C del nuevo módulo "Gestión de usuarios" (Administradores). **`invite_admin` queda confirmado VALIDADO Y FUNCIONAL en Producción, extremo a extremo** (corrige el estado "problema pendiente" reportado en la ronda de diagnóstico anterior de este mismo Sprint).

**Causa raíz del problema de `redirect_to` (ya cerrada)**: el enlace de invitación llegaba con `redirect_to=https://instalaciones.multimax.net/` (raíz del sitio) en vez de `/nueva-contrasena`. Diagnóstico de código (`inviteAdmin()`/`inviteInstalador()`, `admin-operations/index.ts`) confirmó que la lógica de construcción de `redirectTo` ya era correcta (concatena `/nueva-contrasena` cuando el secret `APP_URL` existe) — la causa real era de **configuración de Producción**, no de código: faltaba el secret `APP_URL` y/o la URL exacta en la allowlist de Supabase Auth. **Corregido por el usuario, fuera de este entorno**: secret `APP_URL=https://instalaciones.multimax.net` + `https://instalaciones.multimax.net/nueva-contrasena` agregada en Authentication → URL Configuration → Redirect URLs. Cero cambios de código requeridos ni realizados para esta corrección — confirma la hipótesis del diagnóstico previo.

**Validación real, extremo a extremo, confirmada por el usuario**: `invite_admin` → correo recibido con el `redirect_to` correcto → `/nueva-contrasena` → creación de contraseña inicial (sin pedir ninguna "contraseña actual", como corresponde) → acceso al Dashboard → logout → login con la contraseña recién creada → Dashboard. Mismo mecanismo (`SetPasswordPage.tsx`, sin cambios) ya usado por `invite_instalador` desde el Sprint 6.3 — la corrección de configuración beneficia a ambos flujos por igual (mismo secret `APP_URL` compartido).

**Sprint C (tipos/servicios frontend, sin UI)**: `src/types/database.generated.ts` regenerado vía MCP (`generate_typescript_types`, solo lectura) — estaba desactualizado desde la migración `0011` (Sprint A), no incluía `es_principal` en `TableRow<'admins'>`. `src/types/admin-operations.ts` (+`InviteAdminPayload`/`SetAdminActivoPayload`, espejo exacto del contrato ya desplegado de `admin-operations` v7). `src/services/admin-operations.service.ts` (+`inviteAdmin`/`setAdminActivo`, mismo patrón que `inviteInstalador`/`suspendInstalador`). **Sin tipos/servicios para Coordinadores** — documentado explícitamente por qué: `admin-operations` no tiene ninguna acción de coordinadores desplegada, y `coordinadores` no tiene ningún GRANT a `service_role` ni policy de listado (confirmado sin cambios vía MCP) — declarar ese contrato hoy sería falso.

`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev` verificados en esta ronda — todos limpios (mismos 3 warnings de lint preexistentes, ninguno en archivos de este Sprint; `dev` arrancó correctamente, detenido tras confirmar). Sin RLS/migraciones/Edge Functions/`admin-operations`/backend de Coordinadores tocados. Sin componentes/páginas CRUD. Sin `git commit`/`push`. **Detenido para revisión del usuario.**

## [Gestión de Administradores y Coordinadores — Sprint A "Fundamentos de schema"] — 2026-08-19 — 🟢 Aplicado en Producción vía MCP, validado — DETENIDO PARA REVISIÓN

Primer Sprint del nuevo módulo "Gestión de usuarios" (Administradores + Coordinadores dentro de `AdminPanel`), precedido por dos rondas de análisis/cierre arquitectónico sin implementación (`ANALISIS_GESTION_USUARIOS.md`, aprobadas explícitamente por el usuario antes de tocar cualquier cosa). Sprint A comprende **exclusivamente** el schema base de la distinción Administrador Principal/Secundario — sin `admin-operations`, sin RLS de listado, sin frontend, sin Coordinadores (Sprints B en adelante).

**Precheck (solo lectura, vía MCP)**: confirmó sin discrepancias el estado esperado por el análisis — columna `es_principal` inexistente, índice/trigger/función inexistentes, 1 empresa real (Multimax), 1 admin real (`arnulfo.mendieta@multimax.net`, "Administrador Principal", único admin de esa empresa).

**`supabase/migrations/0011_admins_es_principal.sql`** (aplicada vía MCP): `admins.es_principal boolean not null default false`; índice único parcial `uq_admins_un_principal_por_empresa on admins(empresa_id) where es_principal = true` (garantiza "a lo sumo un Principal por empresa"); backfill genérico (`distinct on (empresa_id) ... order by created_at asc`, sin hardcodear ningún id, según norma explícita de `apply_migration`). El backfill se **mostró primero al usuario** (consulta de solo lectura de `id`/`empresa_id`/`nombre`/`email`/`created_at`/`activo`) y se aplicó solo después de su confirmación explícita — resultado: `arnulfo.mendieta@multimax.net` quedó `es_principal = true`, único candidato posible (único admin real).

**`supabase/migrations/0012_proteger_ultimo_admin_principal.sql`** (aplicada vía MCP): función `proteger_ultimo_admin_principal()` (`security definer`) + trigger `trg_proteger_ultimo_admin_principal` (`before update or delete on admins for each row`) — garantiza "al menos un Principal activo por empresa", cubriendo `DELETE` directo, `UPDATE activo=false`, `UPDATE es_principal=false`, y la cascada real desde `auth.users` (`admins.id references auth.users(id) on delete cascade` — un `auth.admin.deleteUser()` sobre el Principal dispara este mismo trigger vía la cascada, y si es el único activo, aborta toda la operación).

**Validación del backfill** (antes de aplicar 0012, como exigía el orden aprobado): exactamente 1 Principal por empresa (0 empresas sin Principal, 0 con más de uno), índice único parcial confirmado creado correctamente.

**9 pruebas de seguridad**, ejecutadas contra Producción real dentro de transacciones con `ROLLBACK` explícito (sin dejar datos de prueba permanentes, sin crear un segundo Principal permanente — usando un `auth.users` real preexistente sin ninguna fila de rol como sujeto de prueba temporal, dado que `admins.id` tiene FK real a `auth.users.id`): las 9 en **PASS** — eliminar/desactivar/despromover al único Principal activo (directo y vía cascada de Auth) rechazados con `23514`; segundo Principal en la misma empresa (`INSERT` e `UPDATE`) rechazado con `23505`; un administrador secundario se puede crear sin ninguna interferencia del trigger; una empresa de prueba temporal con su propio Principal confirmó el scoping correcto por `empresa_id` (protección independiente por empresa, sin interferencia cruzada); estado real verificado sin cambios después de cada prueba fallida (atomicidad confirmada).

`npm run typecheck`/`npm run build` limpios. Ningún archivo de aplicación (`src/`), Edge Function o RLS tocado — solo las 2 migraciones + esta documentación. Sin `git commit`/`push`. **Detenido para revisión del usuario — no se continuó con el Sprint B.**
