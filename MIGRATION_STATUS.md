...
Última actualización: 2026-07-13 — Sprint 3.10 (`InstallerDashboard`) — ✅ Completado.
...
- **Sprint actual**: 3.11 — 🔜 Preparación del Sprint 3.11 (`InstallerProfile`). Sprint 3.10 (`InstallerDashboard`) — ✅ Completado (cerrado; validación real y visual confirmadas por el usuario).
- **Bloque HTML**: no existe ninguna función `InstallerDashboard` en el HTML fuente — nombre genérico del brief. La función real equivalente es `function Installer(props)` (líneas 3169-3452 del JSX fuente), montada por `App()` cuando `role === "inst"`. Este Sprint reconstruye el subconjunto de esa función alcanzable sin motor de trabajos y sin invadir el alcance ya reservado de `InstallerJobs()` (Sprint 3.12) e `InstallerProfile()` (Sprint 3.11) — ver "Estado" abajo.
- **Estado**: ✅ Completado — el usuario confirmó localmente `npm install`/`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev` y la validación visual (comparación del layout React contra `Multimax_Despacho_v1.3.html`). Análisis previo (ver `docs/sprints/sprint-3.10.md`) confirmó que, de las 284 líneas de `Installer(props)`, solo son reconstruibles ahora: `.mx-instwrap` (teléfono + panel lateral), `.mx-phone-bar`/`.mx-mesel`, `.mx-phonetabs` y la rama `mx-phone-empty` de "Solicitudes" (única alcanzable sin `job`/motor de trabajos, mismo criterio que `Coordinator()` en el Sprint 3.6). `InstallerJobs()`/`InstallerProfile()`, aunque técnicamente reconstruibles con sus propios mocks (`MISJOBS`/`INSTALLERS`), son funciones reales con selector propio ya reservadas como Sprints 3.12/3.11 — no se implementaron aquí.
- **Componente React**: `InstallerDashboard` (`src/components/shared/installer-dashboard.tsx`) — orquestador, sin función real homónima; estado interno `instTab` (reproduce el `useState("solicitudes")` real de `Installer()`); recibe `meId`/`onMeIdChange` como props (en el HTML fuente ese estado vive en `App()`, no en `Installer()`). Componentes auxiliares nuevos: `InstallerSolicitudesEmptyState` (`mx-phone-empty`) y `MxPhoneTabs` (contenedor `.mx-phonetabs`).
- **Datos/utilidades nuevas**: ninguna constante/mock nuevo — reutiliza `INSTALLERS` (ya migrado en Sprint 3.7) para `meInfo` y las opciones del selector `.mx-mesel`.
- **Integración**: `InstallerDashboard` reemplaza la integración ad-hoc del Sprint 3.2.1/3.2.2 (`InstallerSidebar` suelta, con un Phone Placeholder vacío) en `src/layouts/RootLayout.tsx`, dentro de `role === 'instalador'` — aplicada directamente, sin pausar a pedir autorización (regla permanente vigente desde el Sprint 3.9). Nuevo estado `meId`/`setMeId` en `RootLayout` (reproduce `useState("pty")` de `App()`). La integración de `CountRing` (Sprint 3.8) permanece intacta, sin tocar — su lugar real (`mx-alert-h`/`mx-offer-h`) sigue fuera de alcance.
- **Adaptación técnica (no visual)**: `meInfo = INSTALLERS.find(...) ?? INSTALLERS[0]` — fallback no-nulo únicamente para satisfacer TypeScript estricto ante el resultado potencialmente `undefined` de `.find()`; no altera el comportamiento visual.
- **Cobertura del HTML**: de las 284 líneas de JSX fuente de `Installer(props)`, se reconstruyen ~43 líneas (~15%) — barra del teléfono, navegación y estado vacío. CSS nuevo: `.mx-phone-empty` (+ 3 selectores hijos); el resto de clases usadas ya estaban portadas desde Fase 3. Agregado global del proyecto ≈25-26% (ver §7).
- **Archivos creados**: `src/components/shared/installer-dashboard.tsx`, `src/components/shared/installer-solicitudes-empty-state.tsx`, `src/components/shared/mx-phone-tabs.tsx`, `docs/sprints/sprint-3.10.md`.
- **Archivos modificados**: `src/layouts/RootLayout.tsx` (nuevo estado `meId`, reemplazo de la integración temporal), `src/styles/globals.css` (`.mx-phone-empty`), `CHANGELOG.md`, `PROJECT_STATUS.md`, este archivo, `TODO.md`, `docs/SPRINTS_INDEX.md`.
- **Problema encontrado / decisión (reportado, aprobado explícitamente por el usuario al cierre)**: al activar las pestañas "Mis trabajos"/"Perfil", la navegación es real y funcional pero no se renderiza contenido (reservado a los Sprints 3.12/3.11); el `onChange` de `.mx-mesel` omite el reinicio de `step` (fuera de alcance) — ver `docs/sprints/sprint-3.10.md` → "Limitaciones encontradas".
- **Pendientes reales del proyecto** (no bloquean el cierre, ya cerrado, de este Sprint): las 7 ramas restantes de "Solicitudes" (`mx-alert`/`mx-offer`/`mx-phone-sent`/`mx-phone-done`×3) y el consumo de `CountRing` dentro de ellas requieren un Sprint futuro con motor de trabajos real; `InstallerJobs`/`InstallerProfile` quedan para los Sprints 3.12/3.11; el consumidor real de `LiveCountdown` (`statusPill`/`QueueBar` de `Coordinator`) y el resto de `Coordinator()` requieren esa misma base; conectar `onPublish` de `PublishModal`; resolver la desincronización `SucursalSelect`↔`HeaderStatus.sucursalActiva` (heredado de Sprint 3.4); decidir sobre la posible duplicación `ui/Tabs` vs `MxSubtabs`/`MxSubtabButton`/`MxPhoneTabs` para AdminPanel (heredado, sin resolver); `km`/mock `INSTALLERS` sin campo equivalente unificado en `types/domain.ts`/schema SQL (heredado de Sprint 3.2); segunda versión de "Reglas de prioridad" (heredado, candidato natural: Sprint 3.11) — ver notas en `docs/SPRINTS_INDEX.md`. **Sprint 3.10 cerrado (✅ Completado). Sprint 3.11 en preparación, pendiente de su propio análisis previo obligatorio.**
...
- **Fase 5 (Installer)**: pantallas de "Solicitudes" (`mx-alert`/`mx-offer`/`mx-sentcard`/`mx-phone-done`, dependen de `job`/motor de trabajos real), "Mis trabajos" (`mx-myjobs`, reservado Sprint 3.12), Perfil completo (`mx-profscreen`, reservado Sprint 3.11). `CountRing`, ya migrado — Sprint 3.8; `InstallerDashboard` (barra del teléfono, navegación, estado vacío de "Solicitudes"), ya migrado — Sprint 3.10; ambos integrados temporalmente en `RootLayout` hasta que exista el flujo real del Instalador.
...
- **Installer**: `mx-instside` (100%, Sprint 3.2), el shell `PhoneFrame` (cuenta en "Layout"), `CountRing` (100%, Sprint 3.8) y el estado vacío de "Solicitudes" + navegación del teléfono (100%, Sprint 3.10, vía `InstallerDashboard`/`InstallerSolicitudesEmptyState`/`MxPhoneTabs`) ya reconstruidos, integrados temporalmente en `RootLayout` para validación visual; el resto (Solicitudes reales — `mx-alert`/`mx-offer`/`mx-sentcard`/`mx-phone-done` —, Mis trabajos — reservado Sprint 3.12 —, Perfil completo — reservado Sprint 3.11, incl. su propia versión de 4 ítems de "Reglas de prioridad") sigue en 0%.
- **Admin**: 0%
