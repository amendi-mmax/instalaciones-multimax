...
Última actualización: 2026-07-11 — Sprint 3.9 (`LiveCountdown`) — ✅ Completado.
...
- **Sprint actual**: 3.10 — 🔜 Preparación del Sprint 3.10. Sprint 3.9 (`LiveCountdown`) — ✅ Completado (cerrado; validación real, visual y funcional confirmadas por el usuario).
- **Bloque HTML**: `function LiveCountdown({ publishedAt, bidMins })` (líneas 2473-2493 del JSX fuente) — `<span>` de texto con countdown regresivo, timer propio. NO renderiza `CountRing` (Sprint 3.8) y NO dispara callbacks al expirar — ambas asunciones del brief son falsas contra el HTML real, ver "Estado" abajo.
- **Estado**: ✅ Completado — el usuario confirmó localmente `npm install`/`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev` y la validación visual y funcional (timer corriendo en vivo, sin regresiones sobre componentes previamente aprobados). Análisis previo (ver `docs/sprints/sprint-3.9.md`) confirmó que `LiveCountdown` (líneas 2473-2493 del script) es un `<span>` de texto con `useState`+`useEffect`+`setInterval` propios (`Date.now()` en tiempo real, sin la aceleración `CONF.speed` del motor de `jobView`), sin ninguna clase CSS propia. Su único consumidor real es `statusPill(jb)`, helper interno de `Coordinator(props)`, usado dentro de `QueueBar`. Se detectaron y reportaron dos discrepancias del brief contra el HTML real: (1) `LiveCountdown` no renderiza `CountRing` — son componentes de countdown completamente independientes; (2) `LiveCountdown` no dispara ningún callback al expirar — no existe ninguna prop de función en su firma real.
- **Componente React**: `LiveCountdown` (`src/components/shared/live-countdown.tsx`) — estado propio (`rem`), `useEffect`+`setInterval` (limpiado correctamente en el cleanup), `calc` envuelto en `useCallback` (adaptación técnica no visual, ver abajo).
- **Datos/utilidades nuevas**: ninguna constante/mock nuevo. Única utilidad reutilizada, sin duplicar: `fmt` en `src/lib/utils.ts` (JSDoc actualizado con este segundo consumidor real, sin cambio de lógica).
- **Integración**: `LiveCountdown` no tiene todavía consumidor real dentro del flujo de la aplicación — en el HTML fuente su único uso real está dentro de `statusPill(jb)`/`QueueBar`, dentro de `Coordinator()`, que depende de `jobs.length > 0` (sin datos reales todavía, mismo bloqueo documentado para `CoordinatorEmptyState`/`Radar`). A partir de este Sprint aplica la nueva regla permanente del proyecto: la integración temporal forma parte del propio Sprint y no requiere pausar para pedir autorización. Integración TEMPORAL en `src/layouts/RootLayout.tsx`, dentro de `role === 'coordinador'` (su rol real, a diferencia de `CountRing`/`role === 'instalador'`), como último elemento del bloque, con props mock estáticas (`LIVECOUNTDOWN_DEMO_PUBLISHED_AT`/`LIVECOUNTDOWN_DEMO_BID_MINS`) — sin rutas nuevas, sin cambios a React Router, sin lógica de negocio. Se retirará cuando exista el `QueueBar` real.
- **Adaptación técnica (no visual)**: `calc` se envuelve en `useCallback(calc, [publishedAt, total])` para satisfacer la regla de ESLint `react-hooks/exhaustive-deps` (activa desde Fase 3), sin alterar el comportamiento de reinicio del timer del HTML original.
- **Cobertura del HTML**: `LiveCountdown` migrado al 100% en su markup/componente (21 líneas de JSX fuente); cero CSS nuevo (el componente no usa ninguna clase `.mx-*` en el HTML fuente). Agregado global del proyecto ≈25%, sin cambio perceptible respecto al cierre del Sprint 3.8 (ver §7).
- **Archivos creados**: `src/components/shared/live-countdown.tsx`, `docs/sprints/sprint-3.9.md`.
- **Archivos modificados**: `src/lib/utils.ts` (JSDoc de `fmt` actualizado), `src/layouts/RootLayout.tsx` (integración temporal de `LiveCountdown`), `CHANGELOG.md`, `PROJECT_STATUS.md`, este archivo, `TODO.md`, `docs/SPRINTS_INDEX.md`.
- **Problema encontrado / decisión (reportado, aprobado explícitamente por el usuario al cierre)**: `LiveCountdown` se integró temporalmente en `RootLayout.tsx` sin tener todavía un consumidor real — ver "Integración" arriba y `docs/sprints/sprint-3.9.md` → "Problema encontrado / decisión de integración temporal".
- **Pendientes reales del proyecto** (no bloquean el cierre, ya cerrado, de este Sprint): el consumidor real de `LiveCountdown` (`statusPill`/`QueueBar` dentro de `Coordinator`) requiere un Sprint futuro que implemente el motor de trabajos (`jobs`/`publishJob`) real; el consumidor real de `CountRing` (Sprint 3.8, pantallas "alerta"/"oferta" del teléfono del Instalador) requiere un Sprint futuro que implemente el flujo real del Instalador; el resto de `Coordinator()` (`mx-jobcard`, `QueueBar`, `AssignedPanel`, `NoResponsePanel`, respuestas, indicadores) y la posición definitiva de `Radar` dentro de la tarjeta "Despacho en vivo" requieren esa misma base; conectar `onPublish` de `PublishModal` a esa lógica; resolver la desincronización `SucursalSelect`↔`HeaderStatus.sucursalActiva` (heredado de Sprint 3.4); decidir sobre la posible duplicación `ui/Tabs` vs `MxSubtabs`/`MxSubtabButton` para AdminPanel (heredado, sin resolver); `km`/mock `INSTALLERS` sin campo equivalente unificado en `types/domain.ts`/schema SQL (heredado de Sprint 3.2); segunda versión de "Reglas de prioridad" (heredado, sin Sprint asignado) — ver notas en `docs/SPRINTS_INDEX.md`. **Sprint 3.9 cerrado (✅ Completado). Sprint 3.10 en preparación, pendiente de su propio análisis previo obligatorio.**
...
| Líneas de la sección `mx-*` en `globals.css` (incl. comentarios) | 945 — sin cambio en Sprint 3.6 (cero CSS nuevo); +~14 en Sprint 3.7 (bloque `.mx-radar-*` + regla adicional de `prefers-reduced-motion` para clases crudas); sin cambio en Sprint 3.8 (cero CSS nuevo); sin cambio en Sprint 3.9 (cero CSS nuevo) |
