...
Última actualización: 2026-07-10 — Sprint 3.8 (`CountRing`) — ✅ Completado.
...
- **Sprint actual**: 3.9 — 🔜 Preparación del Sprint 3.9. Sprint 3.8 (`CountRing`) — ✅ Completado (cerrado; validación real y visual confirmadas por el usuario).
- **Bloque HTML**: `function CountRing({ remaining, total, size, color })` (líneas 1437-1491 del JSX fuente) — anillo SVG de countdown regresivo. NO depende de `Coordinator`/`mx-jobcard`/`Radar`/Timeline, y NO es el mismo componente que `LiveCountdown` (línea 2473) — ver "Estado" abajo.
- **Estado**: ✅ Completado — el usuario confirmó localmente `npm install`/`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev` y la validación visual (`CountRing` coincide con `Multimax_Despacho_v1.3.html`). Análisis previo (ver `docs/sprints/sprint-3.8.md`, 20 preguntas obligatorias) confirmó que `CountRing` (líneas 1437-1491 del script) es un anillo SVG de countdown, sin ninguna clase CSS propia (todo inline) y sin estado/efectos/timers internos — función pura derivada de sus props. No depende de `Coordinator`, `mx-jobcard`, `Radar` ni "Timeline". Se detectó y reportó un segundo componente real distinto, `LiveCountdown` (línea 2473, con timer propio, usado dentro de `Coordinator`/`mx-jobcard`) — fuera de alcance de este Sprint, sin Sprint numerado todavía.
- **Componente React**: `CountRing` (`src/components/shared/countring.tsx`) — sin estado propio, sin `useEffect`/timers, todo derivado de props (`remaining`/`total`/`size`/`color`), igual que en el HTML fuente.
- **Datos/utilidades nuevas**: ninguna constante/mock nuevo (no usa `INSTALLERS`/`ELIGIBLE_ORDER` ni ningún otro dato compartido). Única utilidad: `fmt` en `src/lib/utils.ts` — reincorporada en este Sprint (se había retirado en el Sprint 3.7 al confirmar que `Radar` no la usaba; `CountRing` sí la usa genuinamente, línea 1482 del HTML fuente).
- **Integración**: `CountRing` no tiene todavía consumidor real dentro del flujo de la aplicación — en el HTML fuente sus dos únicos usos están dentro de `Installer(props)` (pantallas "alerta"/"oferta" del teléfono del Instalador, `mx-alert`/`mx-offer`), que no existen todavía en el proyecto (Sprint 3.2 solo migró `mx-instside`). Siguiendo la instrucción del brief de este Sprint, se detuvo el trabajo, se explicó el motivo y se propuso la integración temporal antes de aplicarla; el usuario la autorizó explícitamente. Integración TEMPORAL en `src/layouts/RootLayout.tsx`, dentro de `role === 'instalador'` (su rol real, a diferencia de `Radar`/`CoordinatorEmptyState`), como hermano de `.mx-instwrap`, con props mock estáticas (`COUNTRING_DEMO_REMAINING`/`COUNTRING_DEMO_TOTAL`) — sin rutas nuevas, sin cambios a React Router, sin lógica de negocio. Se retirará cuando exista el flujo real del Instalador.
- **Cobertura del HTML**: `CountRing` migrado al 100% en su markup/componente (55 líneas de JSX fuente); cero CSS nuevo (el componente no usa ninguna clase `.mx-*` en el HTML fuente, todo es inline). Agregado global del proyecto ≈25%, sin cambio perceptible respecto al cierre del Sprint 3.7 (ver §7).
- **Archivos creados**: `src/components/shared/countring.tsx`, `docs/sprints/sprint-3.8.md`.
- **Archivos modificados**: `src/lib/utils.ts` (reincorporación de `fmt`), `src/layouts/RootLayout.tsx` (integración temporal de `CountRing`), `CHANGELOG.md`, `PROJECT_STATUS.md`, este archivo, `TODO.md`, `docs/SPRINTS_INDEX.md`.
- **Problema encontrado / decisión (reportado, aprobado explícitamente por el usuario)**: `CountRing` se integra temporalmente en `RootLayout.tsx` sin tener todavía un consumidor real — ver "Integración" arriba y `docs/sprints/sprint-3.8.md` → "Problema encontrado / decisión de integración temporal".
- **Pendientes reales del proyecto** (no bloquean el cierre, ya cerrado, de este Sprint): el consumidor real de `CountRing` (pantallas "alerta"/"oferta" del teléfono del Instalador, `mx-phone`) requiere un Sprint futuro que implemente el flujo real del Instalador; `LiveCountdown` queda sin Sprint numerado (candidato: el Sprint que reconstruya `mx-jobcard`/QueueBar); el resto de `Coordinator()` (`mx-jobcard`, `QueueBar`, `AssignedPanel`, `NoResponsePanel`, respuestas, indicadores) y la posición definitiva de `Radar` dentro de la tarjeta "Despacho en vivo" requieren un Sprint futuro que también implemente `jobs`/`publishJob` real; conectar `onPublish` de `PublishModal` a esa lógica; resolver la desincronización `SucursalSelect`↔`HeaderStatus.sucursalActiva` (heredado de Sprint 3.4); decidir sobre la posible duplicación `ui/Tabs` vs `MxSubtabs`/`MxSubtabButton` para AdminPanel (heredado, sin resolver); `km`/mock `INSTALLERS` sin campo equivalente unificado en `types/domain.ts`/schema SQL (heredado de Sprint 3.2); segunda versión de "Reglas de prioridad" (heredado, sin Sprint asignado) — ver notas en `docs/SPRINTS_INDEX.md`. **Sprint 3.8 cerrado (✅ Completado). Sprint 3.9 en preparación, pendiente de su propio análisis previo obligatorio.**
...
| Coordinator · Despacho en vivo (`mx-grid`, JobCard, Radar, Rounds, Feed) | 🟡 Estado vacío (`mx-qempty`, ✅ validado) y `Radar` (✅ validado) reconstruidos, visibles temporalmente en `RootLayout` (ver §0); el resto (`mx-jobcard`, QueueBar, Rounds, Feed) sigue ⛔, depende de `jobs`/`Trabajo` real | Sprint 3.6 (estado vacío) + Sprint 3.7 (`Radar`) / Fase 4 (resto) |
...
| Installer · Solicitudes (Alert/Offer/Sent/Assigned/Lost/Declined/Closed) | 🟡 `CountRing` (anillo de countdown usado en los pasos "alerta"/"oferta") reconstruido y validado (✅), visible temporalmente en `RootLayout` con props mock (ver §0); el resto de las pantallas (`mx-alert`/`mx-offer`/`mx-sentcard`/`mx-phone-done`, flujo real) sigue ⛔ | Sprint 3.8 (`CountRing`) / Fase 5 (resto) |
...
- **Fase 4 (Coordinator)**: JobCard (`mx-jobcard`), RoundsPanel, ResponsesFeed, AssignedPanel, NoResponsePanel, QueueBar, JobList/JobRow, JobDetail (+Timeline), MasterCalendar, `LiveCountdown`, badge master (`mx-master-badge`). Todos estos dependen de `jobs.length > 0` (estado real de `Trabajo`/`publishJob`), todavía inexistente — ver `docs/sprints/sprint-3.6.md`/`docs/sprints/sprint-3.7.md`. (Selector de sucursal, `mx-suc-sel`, ya migrado — Sprint 3.4; `PublishModal`, ya migrado — Sprint 3.5; estado vacío de `Coordinator`, `CoordinatorEmptyState`, ya migrado — Sprint 3.6; `Radar`, ya migrado — Sprint 3.7, integrado temporalmente hasta que exista la tarjeta real "Despacho en vivo"; `LiveCountdown` sin Sprint numerado todavía, distinto de `CountRing` — ver docs/sprints/sprint-3.8.md.)
- **Fase 5 (Installer)**: pantallas de "Solicitudes" (`mx-alert`/`mx-offer`/`mx-sentcard`/`mx-phone-done`), "Mis trabajos" (`mx-myjobs`), Perfil completo (`mx-profscreen`). `CountRing`, ya migrado — Sprint 3.8, integrado temporalmente en `RootLayout` hasta que exista el flujo real del Instalador.
...
| Líneas de la sección `mx-*` en `globals.css` (incl. comentarios) | 945 — sin cambio en Sprint 3.6 (cero CSS nuevo); +~14 en Sprint 3.7 (bloque `.mx-radar-*` + regla adicional de `prefers-reduced-motion` para clases crudas); sin cambio en Sprint 3.8 (cero CSS nuevo) |
...
- **Coordinator**: `mx-suc-sel` (100%, Sprint 3.4), `PublishModal` (100% en markup/CSS/componente, Sprint 3.5, ahora abierto mediante el botón real `onOpenPublish` desde Sprint 3.6), el estado vacío de `Coordinator` (`mx-qempty`, 100%, Sprint 3.6, vía `CoordinatorEmptyState`) y `Radar` (100%, Sprint 3.7, vía `Radar` — único componente SVG, sin sub-componentes; integrado temporalmente en `RootLayout` para validación visual, ver §0) ya reconstruidos; el resto (`mx-jobcard`, `LiveCountdown`, Rounds, Feed, QueueBar, JobList/JobDetail, MasterCalendar) sigue en 0% — depende de `jobs`/`Trabajo` real, ver `docs/sprints/sprint-3.7.md`. (`CountRing`, distinto de `LiveCountdown`, ya migrado en Sprint 3.8 — ver bullet "Installer" abajo, no pertenece a Coordinator.)
- **Installer**: `mx-instside` (100%, Sprint 3.2), el shell `PhoneFrame` (cuenta en "Layout") y `CountRing` (100%, Sprint 3.8, integrado temporalmente en `RootLayout` para validación visual) ya reconstruidos; el resto (Solicitudes reales — `mx-alert`/`mx-offer`/`mx-sentcard`/`mx-phone-done` —, Mis trabajos, Perfil completo — incl. su propia versión de 4 ítems de "Reglas de prioridad") sigue en 0%.
- **Admin**: 0%
