...
- **Sprint actual**: 3.7 — ✅ Completado. **Próximo Sprint**: 3.8 (`Countdown`/`CountRing`), sin iniciar — pendiente de su propio análisis previo obligatorio.
- **Bloque HTML**: `function Radar({ notified, instState, eligibleIds })` (líneas 1492-1745 del JSX fuente) — panel SVG de instaladores notificados. NO es una arquitectura de "mapa" con marcadores/overlays/controles independientes, como sugería el brief — ver "Estado" abajo.
- **Estado**: ✅ Completado — el usuario confirmó localmente `npm install`/`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev` y la validación visual (`Radar` coincide con `Multimax_Despacho_v1.3.html`). Análisis previo (ver `docs/sprints/sprint-3.7.md`) confirmó que `Radar` es un único componente SVG autocontenido, sin ninguna librería de mapas ni sub-componentes propios (`RadarMap`/`RadarMarker`/`RadarOverlay`/etc. no existen en el HTML). Se descartó `CountRing` (vecino en el archivo, líneas 1437-1491) del alcance — es un anillo de countdown sin relación con el radar, reservado para el Sprint 3.8 ("Countdown").
- **Componente React**: `Radar` (`src/components/shared/radar.tsx`) — sin estado propio, todo derivado de props (`notified`/`instState`/`eligibleIds`), igual que en el HTML fuente.
- **Datos/utilidades nuevas**: `INSTALLERS` (11 instaladores) y `ELIGIBLE_ORDER` (9 ids) en `src/constants/index.ts`, verbatim del HTML fuente; `hashAngle` en `src/lib/utils.ts`, utilidad pura verbatim. `fmt` (línea 878 del HTML, vecino de `hashAngle`) se dejó deliberadamente fuera de este Sprint — `Radar` no lo usa.
- **Integración**: `Radar` no tiene todavía consumidor real dentro del flujo de la aplicación — en el HTML fuente solo se monta dentro de la tarjeta "Despacho en vivo" de `Coordinator()`, que requiere `jobs.length > 0` (sin datos reales todavía, mismo bloqueo del Sprint 3.6). Se consultó al usuario explícitamente y se aprobó una excepción: integración TEMPORAL en `src/layouts/RootLayout.tsx`, dentro de `role === 'coordinador'`, después de `CoordinatorEmptyState`, con props mock estáticas — sin rutas nuevas, sin cambios a React Router, sin lógica de negocio. Se retirará cuando exista la tarjeta real.
- **Cobertura del HTML**: `Radar` migrado al 100% en su markup/CSS/componente (254 líneas de JSX fuente); 5 reglas CSS nuevas (`.mx-radar-wrap`/`.mx-radar`/`.mx-sweep`/`.mx-ping`/`.mx-radar-legend`) más un gap de Fase 3 corregido de paso (`prefers-reduced-motion` para clases planas). Agregado global del proyecto ≈25% (ver §7).
- **Archivos creados**: `src/components/shared/radar.tsx`, `docs/sprints/sprint-3.7.md`.
- **Archivos modificados**: `src/constants/index.ts` (`INSTALLERS`/`ELIGIBLE_ORDER`), `src/lib/utils.ts` (`hashAngle`), `src/styles/globals.css` (bloque `.mx-radar-*` + gap de `prefers-reduced-motion`), `src/layouts/RootLayout.tsx` (integración temporal de `Radar`), `CHANGELOG.md`, `PROJECT_STATUS.md`, este archivo, `TODO.md`, `docs/SPRINTS_INDEX.md`.
- **Problema encontrado / decisión (reportado, aprobado explícitamente por el usuario)**: `Radar` se integra temporalmente en `RootLayout.tsx` sin tener todavía un consumidor real — ver "Integración" arriba y `docs/sprints/sprint-3.7.md` → "Problema encontrado / decisión de integración temporal".
- **Pendientes reales del proyecto** (no bloquean el cierre, ya cerrado, de este Sprint): el resto de `Coordinator()` (`mx-jobcard`, `QueueBar`, `AssignedPanel`, `NoResponsePanel`, respuestas, indicadores) y la posición definitiva de `Radar` dentro de la tarjeta "Despacho en vivo" requieren un Sprint futuro que también implemente `jobs`/`publishJob` real; `CountRing` queda para el Sprint 3.8; conectar `onPublish` de `PublishModal` a esa lógica; resolver la desincronización `SucursalSelect`↔`HeaderStatus.sucursalActiva` (heredado de Sprint 3.4); decidir sobre la posible duplicación `ui/Tabs` vs `MxSubtabs`/`MxSubtabButton` para AdminPanel (heredado, sin resolver); `km`/mock `INSTALLERS` sin campo equivalente unificado en `types/domain.ts`/schema SQL (heredado de Sprint 3.2, reforzado en este Sprint); segunda versión de "Reglas de prioridad" (heredado, sin Sprint asignado) — ver notas en `docs/SPRINTS_INDEX.md`. **Sprint 3.7 cerrado (✅ Completado). No se inicia el Sprint 3.8 sin aprobación explícita y sin su propio análisis previo obligatorio.**
...
| Líneas de la sección `mx-*` en `globals.css` (incl. comentarios) | 945 — sin cambio en Sprint 3.6 (cero CSS nuevo); +~14 en Sprint 3.7 (bloque `.mx-radar-*` + regla adicional de `prefers-reduced-motion` para clases crudas) |
