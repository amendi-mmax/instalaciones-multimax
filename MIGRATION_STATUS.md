...
Última actualización: 2026-07-13 — Sprint 3.13 (`AdminPanel`/`AdminInstaladores`) — ✅ Completado.
...
- **Sprint actual**: 3.14 — 🔜 Preparación del Sprint 3.14 (`MasterCalendar`, "Calendar"). Sprint 3.13 (`AdminPanel`/`AdminInstaladores`) — ✅ Completado (cerrado; validación real, visual y funcional confirmadas por el usuario).
- **Bloque HTML**: "Admin Dashboard" (nombre genérico del brief) **no corresponde** a ninguna función real — verificado con `grep -n "function Admin"` sobre el HTML completo. La función real equivalente es `function AdminPanel()` (líneas 3031-3048 del JSX fuente), montada por `App()` cuando `role === "admin"`. `AdminPanel()` compone sub-tabs (`.mx-subtabs-wrap`/`.mx-subtabs`) con dos pestañas: "Calendario maestro" → `MasterCalendar` (función real, sin construir, reservada para el Sprint 3.14 "Calendar") e "Instaladores" → `AdminInstaladores()` (líneas 3049-3160, `.mx-page`/`.mx-pagehead`/`.mx-admingrid`).
- **Estado**: ✅ Completado — el usuario confirmó localmente `npm install`/`npm run lint`/`npm run typecheck`/`npm run build`/`npm run dev`, la validación visual (comparación contra `Multimax_Despacho_v1.3.html`) y la ausencia de regresiones funcionales.
- **Componentes React**: `AdminPanel` (`src/components/shared/admin-panel.tsx`) — orquestador de sub-tabs, `useState('calendario')` (valor inicial idéntico al HTML fuente); la rama "calendario" renderiza `null` en este Sprint (`MasterCalendar` reservado para el Sprint 3.14). `AdminInstaladores` (`src/components/shared/admin-instaladores.tsx`) — reconstrucción verbatim de la tabla de instaladores (Pill de estado dinámico, botón Suspender/Reactivar) + formulario "Invitar instalador".
- **Datos/utilidades**: ninguna constante nueva — reutiliza `INSTALLERS` (Sprint 3.7) y `ZONAS` (Sprint 3.5), ya migradas. Primera aplicación de la regla de "preparación para Supabase" (vigente desde el Sprint 3.12) en un componente del rol Admin.
- **Integración**: real y directa en `src/layouts/RootLayout.tsx` — `{role === 'admin' && <AdminPanel />}`, en la misma posición estructural que usa `App()` en el HTML fuente (después de `role === 'instalador'`, antes de `PublishModal`). No es un mount temporal: el "contenedor" (`RootLayout.tsx` como equivalente de `App()`) ya existía desde el Sprint 3.1.
- **CSS nuevo**: bloque `.mx-admingrid`/`.mx-admintable`/`.mx-adminrow*`/`.mx-admin-act`/`.mx-invite*` (15 selectores + 1 media query), verbatim de las líneas 324-342 y 376 del `<style>` original.
- **Cobertura del HTML**: `AdminPanel`/`AdminInstaladores` reconstruyen ~130 líneas de JSX fuente (`AdminPanel` completo + `.mx-admingrid` de `AdminInstaladores`) + 15 selectores CSS nuevos. Agregado global del proyecto ≈29% (ver §7).
- **Archivos creados**: `src/components/shared/admin-panel.tsx`, `src/components/shared/admin-instaladores.tsx`, `docs/sprints/sprint-3.13.md`.
- **Archivos modificados**: `src/styles/globals.css` (`.mx-admingrid`/`.mx-admintable`/`.mx-adminrow*`/`.mx-admin-act`/`.mx-invite*`), `src/layouts/RootLayout.tsx` (import + `{role === 'admin' && <AdminPanel />}`), `CHANGELOG.md`, `PROJECT_STATUS.md`, este archivo, `TODO.md`, `docs/SPRINTS_INDEX.md`.
- **Decisión de reutilización documentada**: no se usan `Input`/`Select` de `components/ui/` en el formulario de invitación — el HTML fuente estiliza esos campos vía el selector descendiente `.mx-invite input,.mx-invite select`, no vía las clases genéricas `.mx-input`/`.mx-select-native`; se reconstruyeron como elementos nativos.
- **Reportado sin corregir**: la pestaña "Calendario maestro" (activa por defecto) renderiza `null` en este Sprint — mismo criterio ya usado en `InstallerDashboard` (Sprint 3.10) para sus pestañas todavía no implementadas.
- **Pendientes reales del proyecto** (no bloquean el cierre, ya cerrado, de este Sprint): `MasterCalendar` (Sprint 3.14, contenido de la pestaña "Calendario maestro" de `AdminPanel`); las 7 ramas restantes de "Solicitudes" del Instalador y el resto de `Coordinator()`/`TRABAJOS`, que requieren motor de trabajos real; conectar `onPublish` de `PublishModal`; resolver la desincronización `SucursalSelect`↔`HeaderStatus.sucursalActiva` (heredado de Sprint 3.4); decidir sobre la posible duplicación `ui/Tabs` vs `MxSubtabs`/`MxSubtabButton`/`MxPhoneTabs` (heredado, sin resolver, ya no bloquea `AdminPanel` porque este Sprint reutilizó `MxSubtabs`); `km`/mock `INSTALLERS` sin campo equivalente unificado en `types/domain.ts`/schema SQL (heredado de Sprint 3.2) — ver notas en `docs/SPRINTS_INDEX.md`. **Sprint 3.13 cerrado (✅ Completado). Sprint 3.14 en preparación, pendiente de su propio análisis previo obligatorio.**
...
| Admin · Calendario maestro (reutiliza Coordinator) | ⛔ No iniciado — rama `tab === 'calendario'` de `AdminPanel` renderiza `null` hasta el Sprint 3.14 | Sprint 3.14 |
...
- **Fase 6 (Admin)**: `MasterCalendar` (calendario maestro, reservado para el Sprint 3.14, contenido de la rama `tab === 'calendario'` de `AdminPanel`). `AdminPanel`/`AdminInstaladores` (tabla + suspender/reactivar + formulario de invitación) ya reconstruidos y cerrados — Sprint 3.13, ✅ Completado.
...
- **Admin**: 65% — `AdminPanel` (orquestador de sub-tabs, 100%) y `AdminInstaladores` (tabla de instaladores + formulario de invitación, 100%) reconstruidos e integrados de forma real en `RootLayout` (Sprint 3.13, ✅ Completado, validación real y visual/funcional confirmadas por el usuario); `MasterCalendar` (contenido de la pestaña "Calendario maestro") sigue en 0%, reservado para el Sprint 3.14.
