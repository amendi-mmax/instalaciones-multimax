...
└── migrations/
    ├── 0001_initial_schema.sql     # RECONSTRUIDA (Sprint 8.4.1) — schema base real: 8 tablas, funciones, triggers, RLS, índices
    └── 0002_auth_roles_rls.sql     # RETIRADA (Sprint 8.4.1) — no-op, ver cabecera del propio archivo
```

**⚠️ SUPERSEDIDO — ver §11.** El párrafo original de esta sección (modelo `empresas`/`sucursales`/`usuarios`/`zonas_cobertura`/`trabajos`/`bids`/`notificaciones`/`trabajo_instaladores` como "oficial y definitivo") reflejaba una decisión real de Sprint 4.0.1 (tercera ronda), pero quedó **superada por la realidad de Producción** ya desde Sprint 4.0.2 ("Auditoría de Sincronización de Base de Datos") — el `pg_dump` real confirmó que el modelo vigente en Producción siempre fue el de 8 tablas `empresas`/`tiendas`/`admins`/`coordinadores`/`instaladores`/`trabajos`/`trabajo_instaladores`/`ofertas` (mismo aviso que encabeza `PROJECT_STATUS.md` desde esa fecha). El Sprint 8.4.1 reconstruyó por fin `0001_initial_schema.sql` para que coincida con esa realidad — ver §11 para el detalle completo.
...
## 11. Reconstrucción de `0001`/`0002` (Sprint 8.4.1 — "Reconstrucción de Migraciones Supabase")

La confirmación de §10 fue real en su momento, pero la propia Producción del proyecto terminó divergiendo de ella: desde el Sprint 4.0.2 ("Auditoría de Sincronización de Base de Datos") en adelante, toda la infraestructura del repositorio (`src/lib/supabase/config.ts`, `docs/database/DATABASE_INVENTORY.md`, y cada migración `0003` en adelante) ya asume el modelo de 8 tablas `empresas`/`tiendas`/`admins`/`coordinadores`/`instaladores`/`trabajos`/`trabajo_instaladores`/`ofertas` -- el que §10 había descartado. `0001_initial_schema.sql`/`0002_auth_roles_rls.sql`, sin embargo, nunca se actualizaron para reflejar ese giro -- seguían siendo copias byte a byte de lo que hoy vive en `migrations/legacy/`.

Consecuencia real, detectada en la Auditoría de Deployment Readiness que precedió a este Sprint: aplicar `supabase/migrations/*.sql` en orden sobre un proyecto Supabase nuevo fallaba en `0003` ("relation public.admins does not exist"), porque `0001`/`0002` nunca crearon esa tabla.

Este Sprint reconstruyó `0001_initial_schema.sql` a partir del esquema REAL de Producción (verificado exhaustivamente vía MCP -- columnas/tipos/defaults, PK/UNIQUE/FK con su `delete_rule`, índices, las 18 policies RLS previas a `0003`, y 4 funciones + 1 event trigger que resultaron estar activos en Producción sin ningún `CREATE FUNCTION`/`CREATE EVENT TRIGGER` correspondiente en ninguna migración del repositorio -- `set_bid_cierra_at()`, `asignar_instalador()`, `instalador_fue_notificado()`, `submit_bid()`, `rls_auto_enable()`/`ensure_rls`). `0002_auth_roles_rls.sql` quedó retirada (no-op documentado, ver su propia cabecera) -- la numeración `0002`-`0010` no se alteró. Se corrigió además `0008_authenticated_view_grants_sprint72.sql` (único archivo de `0003`-`0010` tocado): otorgaba `GRANT` sobre la vista `trabajos_para_instalador` sin que ningún archivo la creara -- se le agregó su `CREATE OR REPLACE VIEW` (definición exacta, obtenida de Producción real).

**Ninguna de estas correcciones modificó Producción** -- son cambios exclusivamente sobre los archivos del repositorio, sin `db push`/`db reset`/`migration up`, verificados por inspección/simulación lógica de dependencias, no por ejecución real (explícitamente fuera del alcance de este Sprint). El objetivo es que `0001` → `0010`, ejecutadas en orden sobre un proyecto nuevo, reproduzcan exactamente el esquema que Producción ya tiene hoy.
